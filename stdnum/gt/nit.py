...
This number consists of 2 to 12 characters, where the last one is the check
digit (which is a digit or the uppercase letter K) and the rest are digits.
Leading zeroes are usually omitted. Digits and check digit are usually
...
def calc_check_digit(number):
    ...
    weights = (12, 11, 10, 9, 8, 7, 6, 5, 4, 3, 2)
    number = (11 - len(number)) * '0' + number  # Pad with leading zeros.
    total = sum(int(n) * w for w, n in zip(weights, number))
    result = -total % 11
    return str(result) if result != 10 else 'K'
...
def format(number):
    ...
    number = compact(number)
    return '-'.join([number[:-1], number[-1]]).lstrip('0')
